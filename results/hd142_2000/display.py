...
hist(T / log(10.0), 500, alpha=0.4, color="k")
...
true_periods = {
    ...
    "corot7": [0.853592, 3.697, 8.966],  # corot-7  # corot-7  # corot-7
    "hd142": [108.39, 349.7, 6005],  # d  # b  # c
}
...
true_eccs = {
    ...
    "hd142": [0.12, 0.17, 0.21],  # d  # b  # c
}

if star in true_periods.keys():
    for p in true_periods[star]:
        axvline(log(p) / log(10.0), color="g")
ylabel("Number of Posterior Samples")
get_current_fig_manager().set_window_title(os.path.dirname(os.path.realpath(__file__)))
show()
...
plot(T / log(10.0), log10(A), "g.", markersize=1)

subplot(2, 1, 2)
plot(
    log(true_periods[star]) / log(10.0), true_eccs[star], "ko", markersize=7, alpha=0.5
)
xlim([0, 5])
xlabel(r"$\log_{10}$(Period/days)")
ylabel("Eccentricity")
plot(T / log(10.0), E, "g.", markersize=1)
get_current_fig_manager().set_window_title(os.path.dirname(os.path.realpath(__file__)))
show()
